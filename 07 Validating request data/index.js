...
routers._users.post = function(req, res){
    ...
    if((name && email && phone && password) === false){
        return res.send("Please send all required fields", 400)
    }

    // @Todo: Save the user details to file

    // @Todo Respond with a 201 status
    res.send(`Your name is ${userData.name}`)
}
