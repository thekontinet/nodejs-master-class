...
router.get('/users', auth, function(req, res){
    ...
    datastore.find('users', {phone}, function(err, userData){
        ...
        if(!userData) return res.send({message: 'Account Not found'}, 404)
...
router.post('/users', function(req, res){
    ...
    datastore.create('users', userCredentials, function(err){
...
router.put('/users', auth, function(req, res){
    ...
    datastore.find('users', {phone}, function(err, userData){
        if(err || !userData) res.send({err: "data not found"}, 404)
